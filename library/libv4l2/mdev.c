...
static int __media_enum_links(struct media_device *media)
{
    ...
    for (id = 1; id <= media->entities_count; id++) {
        ...
        for (i = 0; i < entity->info.links; ++i) {
            ...
            source = mdev_get_entity_by_id(media, link->source.entity);
            sink = mdev_get_entity_by_id(media, link->sink.entity);
 ...
 * mdev_close when the device isn't needed anymore.
 */
 ...
 */
struct media_entity *mdev_get_entity_by_name(struct media_device *media,
 ...
 * @return A pointer to the entity if found, or NULL otherwise.
 */
struct media_entity *mdev_get_entity_by_id(struct media_device *media,
                        __u32 id)
{
    unsigned int i;
 ...
 */
int mdev_setup_link(struct media_device *media,
...
int mdev_reset_links(struct media_device *media)
{
    ...
    for (i = 0; i < media->entities_count; ++i) {
        ...
        for (j = 0; j < entity->num_links; j++) {
            ...
            ret = mdev_setup_link(media, link->source, link->sink,
 ...
 */
struct media_pad *mdev_parse_pad(struct media_device *media,
                  const char *p, char **endp)
{
    ...
    if (*p == '"') {
        ...
        entity = mdev_get_entity_by_name(media, p + 1, end - p - 1);
        if (entity == NULL)
            return NULL;
        ...
        entity = mdev_get_entity_by_id(media, entity_id);
        if (entity == NULL)
            return NULL;
    }
    for (; isspace(*end); ++end);
 ...
 */
struct media_link *mdev_parse_link(
        struct media_device *media,
        const char *p,
        char **endp)
{
    ...
    source = mdev_parse_pad(media, p, &end);
    if (source == NULL)
        return NULL;
    ...
    sink = mdev_parse_pad(media, p, &end);
    if (sink == NULL)
        return NULL;
        ...
        char **endp)
{
    ...
    link = mdev_parse_link(media, p, &end);
    if (link == NULL) {
    ...
    *endp = (char *)p;
    ...
    return mdev_setup_link(media, link->source, link->sink, flags);
}
...
int mdev_parse_setup_links(struct media_device *media, const char *p)
{
    ...
    do {
        ret = mdev_parse_setup_link(media, p, &end);
        if (ret < 0)
            return ret;
